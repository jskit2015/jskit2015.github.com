...
```js
{
      code:   -1  // 数字,错误码.可选: -1, 未知错误; 1, 对象不存在; 2, 缺乏必须的初始化信息 ;11, 属性不存在; 12, 属性值不合适; 13,属性只读;
}
```
...
```js
{
      code:   -1  // 数字,错误码.可选: -1, 未知错误; 1, 对象不存在; 2, 缺乏必须的初始化信息 ;11, 属性不存在; 12, 属性值不合适; 13,属性只读;31,方法名不存在;32, 缺少必须的参数; 
}
```
...
```js
{
      code:   -1  // 数字,错误码.可选: -1, 未知错误; 1, 对象不存在; 2, 缺乏必须的初始化信息 ;11, 属性不存在; 12, 属性值不合适; 13,属性只读;31,方法名不存在;32, 缺少必须的参数; 41, 事件不支持; 
}
```
